...
namespace AVS.CoreLib.Console.ColorFormatting.Tags
{
    ...
    public class RgbTagProcessor : TagProcessor
    {
        ...
        //private static bool TryParse0(StringBuilder sb, out (byte R, byte G, byte B) rgb, out string rgbText)
        //{
        //    var ind = sb.IndexOf("<RGB:");
        //    var parts = sb.ToString(ind + "<RGB:".Length, 12).Split(new[] { ',', '>' }, StringSplitOptions.RemoveEmptyEntries);

        //    if (parts.Length > 2 && byte.TryParse(parts[0], out var r) && byte.TryParse(parts[1], out var g) && byte.TryParse(parts[2], out var b))
        //    {
        //        rgb = (r, g, b);
        //        rgbText = sb.ToString(ind, 8 + parts[0].Length + parts[1].Length + parts[2].Length);
        //        return true;
        //    }
