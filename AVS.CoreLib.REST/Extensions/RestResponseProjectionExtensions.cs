...
public static class RestResponseProjectionExtensions
{
    ...
    /// <param name="func">gives control to the caller's code over projection (Map / MapArray / MapDictionary)</param>
    /// <returns><see cref="Response{T}"/></returns>
    public static Response<T> ToResponse<T>(this RestResponse restResponse, Func<Proj<T>, T?> func)
    {
        var isSuccess = restResponse.IsSuccessful();
        var response = Response.Create<T>(restResponse.Source, content: restResponse.Content, restResponse.Error, restResponse.Request);
    ...
    /// <param name="func">gives control to the caller's code over projection (Map / MapArray / MapDictionary)</param>
    /// <returns><see cref="Response{T}"/></returns>
    public static Response<T> ToResponse<T, TType>(this RestResponse restResponse, IProxy<TType, T> proxy, Func<ProxyProj<T>, T?> func)
    {
        var isSuccess = restResponse.IsSuccessful();
        var response = Response.Create<T>(restResponse.Source, content: restResponse.Content, restResponse.Error, restResponse.Request);
    ...
    /// <param name="func">gives control to the caller's code over projection (Map / MapArray / MapDictionary)</param>
    /// <returns><see cref="Response{T}"/></returns>
    public static Response<T> ToResponse<T, TType>(this RestResponse restResponse, IKeyedCollectionProxy<T, TType> proxy, Func<KeyedProxyProj<T>, T?> func)
    {
        var isSuccess = restResponse.IsSuccessful();
        var response = Response.Create<T>(restResponse.Source, content: restResponse.Content, restResponse.Error, restResponse.Request);
